...
window.populateCriteriaCards = function() {
  ...
  try {
    ...
    if (totalPages > 1) {
      ...
      paginationFooter.className = `criteria-pagination-footer flex ${isMobile ? 'flex-col gap-2' : 'justify-between items-center'} mt-4 p-${isMobile ? '2' : '3'} rounded-lg`;
      paginationFooter.innerHTML = `
        <div class="${isMobile ? 'text-xs' : 'text-sm'} text-gray-600 ${isMobile ? 'text-center' : ''}">
        ...
        <div class="flex items-center ${isMobile ? 'justify-center' : ''} space-x-2">
          <button onclick="changeCriteriaPage(-1)" 
                  ...
                  class="px-${isMobile ? '2' : '3'} py-1 ${isMobile ? 'text-xs' : 'text-sm'} bg-white hover:bg-gray-100 border border-gray-300 rounded-lg transition-colors">
            ← Prev
          </button>
          <span class="${isMobile ? 'text-xs' : 'text-sm'} text-gray-700 font-medium px-2">${currentPage + 1} / ${totalPages}</span>
          <button onclick="changeCriteriaPage(1)" 
...
function simplePopulate(container, showDistance) {
  try {
    ...
    if (totalPages > 1) {
      ...
      paginationContainer.className = 'w-full mt-6 flex justify-between items-center rounded-lg p-4';
...
window.populateBookingTherapistInfo = function(therapist) {
  ...
  try {
    ...
    // Create compact therapist info display (no rating, no cost, smaller size)
    container.innerHTML = `
      <div class="flex items-center gap-3">
        <div class="flex-shrink-0">
          <img src="${therapist.image || 'User Panel Media/therapist1.png'}" alt="${fullName}" class="w-16 h-16 rounded-full object-cover border-2 border-gray-200">
        </div>
        
        <div>
          <h3 class="text-base font-medium text-gray-600">${fullName}</h3>
          <p class="text-sm text-gray-600">${approaches}</p>
          <p class="text-xs text-gray-500">${locationText}</p>
          <p class="text-xs text-green-600 font-medium">${therapyFormatText} Therapy</p>
        </div>
      </div>
    `;
