...
window.selectTherapistAndGoToBooking = function(therapistId) {
  ...
  if (bookingStep) {
    ...
    // Initialize booking step with therapist data using the proper function
    if (typeof window.initializeBooking === 'function') {
      console.log('🔧 SELECT: Initializing booking with proper function...');
      window.initializeBooking(therapist);
    } else {
      console.error('❌ SELECT: initializeBooking function not found');
      // Fallback: try to populate individually
      if (typeof window.populateTherapistInfo === 'function') {
        window.populateTherapistInfo(therapist);
      }
      if (typeof window.populateServiceSelection === 'function') {
